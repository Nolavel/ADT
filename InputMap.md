...
|---|---|---|
| `weapon_reload` | Оставляем как есть / kept as-is | Висит на R (physical keycode 82) — раньше конфликтовал с `Interact`, конфликт снят переносом `Interact` на F. Оружия в игре пока нет — action держим про запас. |
| `toggle_scanner` (T) | 🗑 Мусор / stale — удалить | Заготовка без потребителя, назначение неясно. |
| `Spaceship` (K) | 🗑 Мусор / stale — удалить | Заготовка без потребителя, назначение неясно, к текущему сеттингу (ховер-транспорт, без космических кораблей) не относится. |
| `debug_info` | Оставляем / kept | Служебный дебаг-хоткей, не относится к геймплейному InputMap. |

---

## 8. История решений / Decision log

- `Interact`: R → **F**. Причина: R конфликтовал одновременно с орбитальной
  камерой (после первичного варианта E) и с существующим `weapon_reload`.
  F не пересекается ни с чем.
- Орбитальная камера (Q/E), зум (колесо), `toggle_view` (V), `toggle_follow`
  (P) — оставлены без изменений по явному запросу, во избежание регрессий в
  отлаженной камерной системе.
- VEHICLE_HOVER — управление клавиатурой (WASD/Shift/F), переиспользуя
  существующие action'ы ON_FOOT, а не заводя дублирующие `vehicle_*` action'ы
  — решение по итогам обсуждения.
- `toggle_scanner` и `Spaceship` — помечены на удаление как мусорные
  action'ы, не привязанные ни к какому текущему или запланированному
  геймплейному элементу.
