...
## Точка появления игрока. Рядом с тестовым кластером (ночлежка, толпа,
## ховер, дроны), чтобы прогон начинался у того, что проверяется, а не в
## пятистах метрах от него. Рельеф в этой точке 130.1 м по запечённой карте
## высот (с учётом сдвига TerrainStaticBody 0.55), отсюда 133 — три метра
## запаса на падение.
##
...
var spawn_point: Vector3 = Vector3(220.0, 133.0, -140.0)
