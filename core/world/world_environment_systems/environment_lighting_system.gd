...
#
# ПРОФИЛИ ПОД СТРАТЫ: сейчас намеренно один профиль на весь город —
# естественное затенение даёт плотная застройка. Если понадобится
# надстройка: подписка на WorldSystems.strata_changed + бленд между
# двумя .tres. Архитектура это допускает без переделки.
#
