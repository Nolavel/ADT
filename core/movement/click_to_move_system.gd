...
#
# NOT gated by Stance. player.gd's punch handler listens on the same
# InputSystems.primary_click_pressed signal this system's own stop/cancel
# handler does, but the two never conflict: player.gd only acts in COMBAT,
# and its own movement lock while punching only ever composes with this
# system's stop-movement effect, never races it. mouse_right_button (the
# move order) is a different button entirely and was never in question.
# =============================================================================
extends Node
