# =============================================================================
# menu_system.gd — обычная нода, НЕ автозагрузка.
#
# Владелец: world.gd — создаёт через .new() и держит как дочернюю ноду.
# Раньше жил в автозагрузке — там preload() сцены меню отрабатывал ДО того,
# как редактор успевал полностью переиндексировать ресурсы после git pull,
# что и приводило к "Parse Error: ext_resource non-existent" на
# menu_controller.gd. Как обычная нода, создаётся уже во время игры
# (после _init_world()), когда все UID точно проиндексированы.
#
...
# сигнал InputSystems.pause_pressed (InputSystems остаётся автозагрузкой
# и сам ничего не решает, только сообщает "кнопка pause нажата").
#
