...
- `INPUT_MAP.md` is the **single source of truth for input bindings** (verified against `project.godot` 2026-09-02, 32 actions) — update it whenever an action is added/removed/rebound in Project Settings → Input Map (`project.godot` `[input]`).
...
- **`ViewMode` is a FRAMING, and only the camera may read it.** Since 2026-09-02 there is one on-foot camera; `TPS` and `TPS_WIDE` are the same camera at the same distance with a different lens shift. Movement, mouse capture, head-look, the cursor and the HUD decals all had `view_mode` branches and all lost them in that commit. A new branch on `view_mode` outside `OnFootCameraComponent` is almost certainly a mistake — say what it is for before writing it.
- **Only `InputSystems.gd` reads `Input` directly, and only it WRITES `Input.mouse_mode`.** Exceptions: `map_source/` and `map_camera/` — these are intentional level-design tools using raw `KEY_*` input; do not refactor them. The write half is stated because it was broken and cost a playtest: `MouseCursorUI` also set `mouse_mode` (HIDDEN on foot), the two writers raced, and HIDDEN keeps the pointer inside the window — so the camera stopped turning at the screen edge. Measured 2026-09-02 as `mouse_mode` 0 where `InputSystems` had asked for 2.
- **A gesture's state machine may not ask "has it ended" before "has it started".** `AnimationTree` updates on the IDLE frame while `player.gd`'s punch/shot/reload machines run in `_physics_process`, so "the one-shot is not active" and "it has not started yet" are the same reading until an idle frame passes. A one-frame grace is not enough at the project's own 55 FPS / 60 Hz, and never enough under the render probe. Each machine latches a `*_gesture_seen` flag first and only then tests for the end, with `GESTURE_START_GRACE` as the backstop for a gesture that never starts. Adding a fourth gesture means adding that flag too — without it the gesture ends on its second physics frame and whatever it was going to do never happens.
- **Inside `InputSystems`: edges come from events, levels come from polls.** A discrete press or release is matched on the `InputEvent` (keyboard in `_unhandled_input()`, mouse in `_input()` — see `docs/architecture/autoloads_and_bootstrap.md` for why they differ); held state and axes stay as `Input.is_action_pressed()` / `get_vector()` in `_physics_process()`. Do not add a new `Input.is_action_just_pressed()` anywhere: polling an edge from the physics frame drops presses as soon as the idle and physics rates diverge (measured 2026-09-02 — 120 taps arrive as 42 at 5 Hz physics, and as 120 through the event path at every rate tested). A query method that has to stay a poll is answered from the edge latch in that file, never from `Input` directly. **That latch carries no frame arithmetic**: an event files the action, the top of the next physics frame promotes it to readable, and the frame after that drops it. The first version stored `Engine.get_physics_frames()` and expired against it, which needs the flush order to be what I assumed it was — measured 2026-09-02, six taps produced six latches and zero reads, and every polled edge in the project (V, jump, stance, lock-on, shoulder) was dead.
