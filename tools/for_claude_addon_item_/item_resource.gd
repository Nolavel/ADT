...
#
# 2026-08-27 — added `pose: ItemPose`. Placement of a held item moves from
# global offsets on EquipmentVisualsComponent onto the item itself. Null
# remains legal and falls back to the old exports so existing .tres keep
# working until they are authored.
# =============================================================================
...
## How this item sits in the hands.
##
## Null is allowed and means "use the global fallback offsets on
## EquipmentVisualsComponent". That keeps every existing .tres working
## without a mass re-author. Once a pose is assigned, the global offsets
## are ignored for this item.
##
## Authored by the Item Pose Editor plugin (or by hand in the inspector).
## The same Resource is what the runtime applies — there is no second
## table of offsets.
@export var pose: ItemPose = null
